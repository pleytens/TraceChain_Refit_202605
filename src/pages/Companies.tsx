...
const CompanyModal: React.FC<ModalProps> = ({ company, onClose, onSave }) => {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-1 px-6 pt-4">
          {(["info", "account"] as const).map((tab) => (
            ...
            >
              {tab === "info" ? "Company Information" : "Account"}
            </button>
          ))}
        </div>
...
const Companies: React.FC = () => {
  ...
  const filtered = companies.filter((c) =>
    !search || c.name.toLowerCase().includes(search.toLowerCase()) || c.gs1Code.includes(search)
  );
  ...
  const handleSave = (data: Partial<Company>) => {
    if (editCompany) {
      ...
      setCompanies([...companies, { id: Date.now(), createdAt: new Date().toISOString().slice(0, 10), ...data } as Company]);
    }
  };

  const handleDelete = (id: number) => {
    if (confirm("Delete this company?")) setCompanies(companies.filter((c) => c.id !== id));
  };

  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <h2 className="text-base font-semibold text-gray-800">Companies</h2>
        <button
