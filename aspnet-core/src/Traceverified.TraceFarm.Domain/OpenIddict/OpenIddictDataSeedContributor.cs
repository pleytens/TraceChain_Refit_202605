...
namespace Traceverified.TraceFarm.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("TraceFarm") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor
            {
                Name = "TraceFarm", DisplayName = "TraceFarm API", Resources = { "TraceFarm" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["TraceFarm_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["TraceFarm_Web:RootUrl"].EnsureEndsWith('/');

            /* TraceFarm_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                "Web Application",
                configurationSection["TraceFarm_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        var consoleAndAngularClientId = configurationSection["TraceFarm_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["TraceFarm_App:RootUrl"]?.TrimEnd('/');
            await CreateApplicationAsync(
        ...
        // Blazor Client
        var blazorClientId = configurationSection["TraceFarm_Blazor:ClientId"];
        if (!blazorClientId.IsNullOrWhiteSpace())
        {
            var blazorRootUrl = configurationSection["TraceFarm_Blazor:RootUrl"]?.TrimEnd('/');
        ...
        var blazorServerTieredClientId = configurationSection["TraceFarm_BlazorServerTiered:ClientId"];
        if (!blazorServerTieredClientId.IsNullOrWhiteSpace())
        {
            var blazorServerTieredRootUrl =
                configurationSection["TraceFarm_BlazorServerTiered:RootUrl"].EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                configurationSection["TraceFarm_BlazorServerTiered:ClientSecret"] ?? "1q2w3e*",
                new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["TraceFarm_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["TraceFarm_Swagger:RootUrl"]?.TrimEnd('/');
