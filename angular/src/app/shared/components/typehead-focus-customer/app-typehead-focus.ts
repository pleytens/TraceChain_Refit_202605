...
@Component({
  selector: 'app-typehead-focus-customer',
  standalone: true,
...
export class TypeheadFocusCustomerComponent implements OnInit, OnChanges {
  @Input() dataInput: any;
  ...
  search: OperatorFunction<string, readonly string[]> = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
      map(term =>
        (term === ''
          ? this.dataInput.data
          : this.dataInput.data.filter(v => v.name.toLowerCase().indexOf(term.toLowerCase()) > -1)
        ).slice(0, 100)
      )
    );
  };
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.initialSelected && !changes.initialSelected.isFirstChange()) {
      this.initializeModel();
    }
  }
